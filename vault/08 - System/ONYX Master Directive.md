...
16. **Enforce fractal linking.** Every note has exactly one `up:` pointing at its direct parent. Leaves (phases, directives, logs, per-item overviews) point at their hub, not at the project Overview. Hubs point at their Overview. The body `## 🔗 Navigation` block is UP-only — a single link to the parent. Sideways links to siblings (Knowledge, Kanban, other hubs) are forbidden in nav blocks; they belong in body prose if needed. Full spec: [[08 - System/Conventions/Fractal Linking Convention.md|Fractal Linking Convention]]. Healer skill: [[08 - System/Agent Skills/_onyx-runtime/heal-fractal-links/heal-fractal-links.md|heal-fractal-links]] (invoked from [[08 - System/Operations/heal.md|heal]] step 7).
