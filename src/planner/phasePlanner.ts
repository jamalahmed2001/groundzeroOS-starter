...
// Select the appropriate system prompt based on project profile.
// Non-code profiles get a domain-neutral planner that doesn't reference repo structure.
function buildPlannerSystemPrompt(profileName: string, writeDirect: boolean): string {
  const isCodeProfile = profileName === 'engineering' || profileName === 'trading';
  if (isCodeProfile) {
    return writeDirect ? PHASE_PLANNER_WRITE_SYSTEM_PROMPT : PLANNER_SYSTEM_PROMPT;
  }
  if (writeDirect) {
    return `You are a project architect decomposing a ${profileName} project into phases.

You will be given:
- A project Overview
- A bundle directory to explore (contains context docs, directives, prior phase notes)
- A target directory to write phase notes into

Your task: decompose the project into 4-8 phases appropriate for this domain and write them as markdown files.
...
<2-3 sentence summary of what this phase produces and why it comes at this position>

## 🧠 Context Pack (do not skip)

<Key facts the agent must know: conventions, constraints, prior work, output location.>

## ✅ Acceptance Criteria

- [ ] <Measurable done condition — output artifact exists at the specified location>
- [ ] <Another criterion>
...
Rules:
- 4-8 phases maximum
- Each phase must produce a clear, independently reviewable output artifact
- Phases ordered by dependency (earlier phases unblock later ones)
- Phase name: 3-6 words, action-oriented (e.g. "Research competitor landscape", "Write episode scripts")
...
[
  {
    "number": 1,
    "name": "Short phase name",
    "summary": "2-3 sentences on what this phase produces and why it comes first.",
    "context": "Key facts the agent must know: conventions, constraints, prior work to build on.",
    "acceptance_criteria": ["Measurable done condition", ...]
  },
  ...
]`;
...
export async function planPhases(
  ...
  const isCodeProfile = profileName === 'engineering' || profileName === 'trading';
  const repoPath = isCodeProfile ? resolveRepoPath(bundle, config) : '';
  const exploreDir = repoPath || bundle.bundleDir;
  ...
  const repoSection = planningUsesAgent(config, exploreDir)
    ? `You have full read access at ${exploreDir} — use Read, Glob, and Grep as needed.`
    : isCodeProfile
      ? `Repo file structure (${repoPath || 'path unknown'}):\n${repoTree}`
      : `Bundle directory: ${exploreDir}\nRead context docs, directives, and any existing files here to understand the project.`;
  ...
  if (planningUsesAgent(config, exploreDir)) {
    ...
    const exploreLabel = isCodeProfile ? 'Repo' : 'Bundle';
...
You have full read access at ${exploreDir}. Read key files before planning.
...
Use the phase note format from your system prompt. Write files directly.`;
    ...
    const result = await spawnClaudeCode({
      prompt: agentPrompt,
      repoPath: exploreDir,
      systemPrompt: config.prompts?.decompose ?? buildPlannerSystemPrompt(profileName, true),
      timeoutMs: 300_000,
  ...
  const output = await planningCall({
    config,
    repoPath: exploreDir,
    systemPrompt: config.prompts?.decompose ?? buildPlannerSystemPrompt(profileName, false),
    userPrompt,
    maxTokens: 3000,
  });
...
export async function planNewPhases(
  ...
  // 4. Scan repo file tree (code profiles only)
  const { profileName: extProfileName, profileSection: extProfileSection } = loadProfileContext(bundle, config);
  const extIsCodeProfile = extProfileName === 'engineering' || extProfileName === 'trading';
  const repoPath = extIsCodeProfile ? resolveRepoPath(bundle, config) : '';
  const exploreDir = repoPath || bundle.bundleDir;
  ...
  const repoTree = extIsCodeProfile ? scanRepoFiles(repoPath) : '';
  const repoSection = planningUsesAgent(config, exploreDir)
    ? `You have full read access at ${exploreDir} — use Read, Glob, and Grep as needed to understand what has been built.`
    : extIsCodeProfile
      ? `Repo file structure (${repoPath || 'path unknown'}):\n${repoTree}`
      : `Bundle directory: ${exploreDir}\nRead context docs and any existing artifacts here.`;
...
Propose additional phases starting from P${nextNum}. The phases must continue logically from the existing work and reflect the current Overview scope.`;
  ...
  if (planningUsesAgent(config, exploreDir)) {
    const today = new Date().toISOString().slice(0, 10);
    const logsDir = path.join(bundle.bundleDir, 'Logs');
    const extExploreLabel = extIsCodeProfile ? 'Repo' : 'Bundle';
    const agentPrompt = `Project: ${bundle.projectId}
...
You have full read access at ${exploreDir}. Read key files to understand what has been built.
...
Use the phase note format from your system prompt. Write files directly.`;
    ...
    const result = await spawnClaudeCode({
      prompt: agentPrompt,
      repoPath: exploreDir,
      systemPrompt: config.prompts?.decompose ?? buildPlannerSystemPrompt(extProfileName, true),
      timeoutMs: 300_000,
      model: config.llm.model,
    });
